...
public class PlayerControlOriginal : MonoBehaviour
{
    ...
    private ItemRootOriginal item_root = null; // ItemRoot 스크립트를 가짐.
    public GUIStyle guistyle; // 폰트 스타일.

    private GameObject closest_event = null;// 주목하고 있는 이벤트를 저장.
    private EventRootOriginal event_root = null; // EventRoot 클래스를 사용하기 위한 변수.
    private GameObject rocket_model = null; // 우주선의 모델을 사용하기 위한 변수.

    private GameStatusOriginal game_status = null;
    ...
    void Start()
    {
        ...
        this.item_root = GameObject.Find("GameRoot").GetComponent<ItemRootOriginal>();
        this.guistyle.fontSize = 16;

        this.event_root =
        GameObject.Find("GameRoot").GetComponent<EventRootOriginal>();
        this.rocket_model = GameObject.Find("Rocket").transform.Find(
        "RocketModel").gameObject;

        this.game_status =
            GameObject.Find("GameRoot").GetComponent<GameStatusOriginal>();
                ...
                case STEP.MOVE: // '이동 중' 상태의 처리.
                    do
                    {
                        ...
                        if (this.closest_event != null)
                        {
                            ...
                            // 이벤트 종류를 가져온다.
                            EventOriginal.TYPE ignitable_event =
                                this.event_root.getEventType(this.closest_event);
                            switch (ignitable_event)
                            {
                                case EventOriginal.TYPE.ROCKET: // 이벤트의 종류가 ROCKET이면.
                                                        // REPAIRING(수리) 상태로 이행.
                        ...
                        {
                            // 가지고 있는 아이템 판별.
                            ItemOriginal.TYPE carried_item_type =
                                this.item_root.getItemType(this.carried_item);
                            switch (carried_item_type)
                            {
                                case ItemOriginal.TYPE.APPLE: // 사과라면.
                                case ItemOriginal.TYPE.PLANT: // 식물이라면.
                                                      // '식사 중' 상태로 이행.
    ...
    void OnGUI()
    {
        ...
        if (this.carried_item != null)
        {
            ...
            do
            {
                ...
                if (item_root.getItemType(this.carried_item) == ItemOriginal.TYPE.IRON)
                {
                    break;
                }
